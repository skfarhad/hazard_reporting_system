...
ALTER TABLE adm1_division
    ...
    ADD CONSTRAINT fk_admin0_pcode FOREIGN KEY (admin0_pcode_fkey) 
        REFERENCES adm0_country(admin0_pcode);

-- 3. Update the new column by removing 'BD' prefix and converting to SMALLINT
UPDATE adm1_division
SET admin1_pcode_fkey = CAST(SUBSTRING(
            admin1_pcode FROM 3 FOR CHAR_LENGTH(admin1_pcode) - 2) AS SMALLINT);
...
ALTER TABLE adm2_district
    ...
    ADD CONSTRAINT fk_admin1_pcode FOREIGN KEY (admin1_pcode_fkey) 
                                    REFERENCES adm1_division(admin1_pcode_fkey),
    ADD CONSTRAINT fk_admin0_pcode FOREIGN KEY (admin0_pcode_fkey) 
                                    REFERENCES adm0_country(admin0_pcode);

-- 3. Update the new column by removing 'BD' prefix and converting to SMALLINT
UPDATE adm2_district
SET admin2_pcode_fkey = CAST(SUBSTRING(
            admin2_pcode FROM 3 FOR CHAR_LENGTH(admin2_pcode) - 2) AS SMALLINT),
    admin1_pcode_fkey = CAST(SUBSTRING(
            admin1_pcode FROM 3 FOR CHAR_LENGTH(admin1_pcode) - 2) AS SMALLINT);
...
ALTER TABLE adm3_upazila
    ...
    ADD CONSTRAINT fk_admin2_pcode FOREIGN KEY (admin2_pcode_fkey) 
                                    REFERENCES adm2_district(admin2_pcode_fkey),
    ADD CONSTRAINT fk_admin1_pcode FOREIGN KEY (admin1_pcode_fkey) 
                                    REFERENCES adm1_division(admin1_pcode_fkey),
    ADD CONSTRAINT fk_admin0_pcode FOREIGN KEY (admin0_pcode_fkey) 
                                    REFERENCES adm0_country(admin0_pcode);

-- 3. Update the new column by removing 'BD' prefix and converting to INTEGER
UPDATE adm3_upazila
SET admin3_pcode_fkey = CAST(SUBSTRING(
            admin3_pcode FROM 3 FOR CHAR_LENGTH(admin3_pcode) - 2) AS INTEGER),
    admin2_pcode_fkey = CAST(SUBSTRING(
            admin2_pcode FROM 3 FOR CHAR_LENGTH(admin2_pcode) - 2) AS SMALLINT),
    admin1_pcode_fkey = CAST(SUBSTRING(
            admin1_pcode FROM 3 FOR CHAR_LENGTH(admin1_pcode) - 2) AS SMALLINT);
...
-- 2. Add unique constraint and set foreign key constraints
ALTER TABLE adm4_thana_union
    ADD CONSTRAINT fk_admin3_pcode FOREIGN KEY (admin3_pcode_fkey) 
                                    REFERENCES adm3_upazila(admin3_pcode_fkey),
    ADD CONSTRAINT fk_admin2_pcode FOREIGN KEY (admin2_pcode_fkey) 
                                    REFERENCES adm2_district(admin2_pcode_fkey),
    ADD CONSTRAINT fk_admin1_pcode FOREIGN KEY (admin1_pcode_fkey) 
                                    REFERENCES adm1_division(admin1_pcode_fkey),
